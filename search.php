...
get_sidebar(); ?>
	<div id="mdclcntr_content" role="main">
		<?php $count = 0;
		if ( have_posts() ) : ?>
			<h2 class="mdclcntr_title_cat"><?php printf( __( 'Search results for:', 'medical-center' ) . '&nbsp;' . '%s', get_search_query() ); ?></h2>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					<header>
						...
						<div class="mdclcntr_meta">
							...
							<a href="<?php the_permalink(); ?>" title="Permalink"><?php echo get_the_date( 'j F, Y' ); ?></a>
					...
					<div class="mdclcntr_entry_content">
						...
						wp_link_pages();
						if ( 0 != $count ) : ?>
							<div class="link_top"><a href="javascript:scroll(0,0);"><?php _e( '[Top]', 'medical-center' ); ?></a>
							</div>
						<?php endif; ?>
			...
			<article class="post-0 post-not-found">
				<h2><?php _e( 'Nothing Found', 'medical-center' ); ?></h2>
				<div class="mdclcntr_entry_content">
					<p><?php _e( 'Sorry, unfortunately, we could not find the requested page.', 'medical-center' ); ?></p>
					<?php get_search_form(); ?>
				</div><!-- mdclcntr_entry_content -->
			</article>
		<?php endif; ?>
